...
export default async function YachtPricingPage({ params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  return (
    <div className="flex flex-col gap-6 h-full max-w-[1600px] mx-auto animate-in fade-in duration-500">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h2 className="text-xl font-semibold text-slate-900">Manage Pricing Rules</h2>
          <p className="text-sm text-slate-500">Configure dynamic pricing rules for {yacht.name}</p>
        </div>
      </div>

      <div className="flex-1 min-h-0">
        <PricingRuleManager yacht={yacht} />
      </div>
    </div>
  );
}
